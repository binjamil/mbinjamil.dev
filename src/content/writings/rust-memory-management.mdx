...
1. ownership, makes heap memory feel like stack memory if its rules are followed
1. 1. every heap-allocated object has only one owner
1. 1. when owner goes out of scope, memory is dropped (leaks are highly unlikely in Rust)
1. 1. code snippet from brown using Box::new
1. borrowing, to not move ownership
1. 1. creates references to the value owned by someone else
1. 1. references are valid as long as owner is not dropped

Binding the allocation lifetime of a resource, such as memory, to the lifetime of an object in this way is known as _Resource Acquisition Is Initialization (RAII)_.
...
```
Here's the summary from `valgrind`:
